(function () {
  ...
  function renderJakeLayout(rawData) {
    ...
    const configs = [
      { margin: 34, nameSize: 18, bodySize: 10, smallSize: 9.2, sectionSize: 11.5, lineHeight: 1.15, headerGap: 8, sectionGap: 10, bulletGap: 2, jobGap: 6 },
      { margin: 30, nameSize: 17, bodySize: 9.5, smallSize: 8.8, sectionSize: 11, lineHeight: 1.1, headerGap: 6, sectionGap: 8, bulletGap: 1.5, jobGap: 5 },
      { margin: 26, nameSize: 16, bodySize: 9, smallSize: 8.3, sectionSize: 10.5, lineHeight: 1.05, headerGap: 5, sectionGap: 7, bulletGap: 1, jobGap: 4 }
    ];
    ...
    function render(doc, cfg) {
      ...
      function ensureSpace(height) {
        if (y + height > pageHeight - cfg.margin) {
          doc.addPage();
          y = cfg.margin;
        }
      }
      ...
      function addSection(title) {
        ...
        doc.text(sanitizePdfText(title).toUpperCase(), cfg.margin, y);
        y += 4;
      ...
      function addBullet(text) {
        ...
        ensureSpace(height + cfg.bulletGap);
        doc.circle(cfg.margin + 2, y - cfg.bodySize / 3, 1.1, 'F');
        doc.text(lines, cfg.margin + 8, y);
        y += height + cfg.bulletGap;
      }

      function addHeaderRow(primary, rightText, secondary) {
        ensureSpace(cfg.bodySize * cfg.lineHeight * 2 + 2);
        doc.setFontSize(cfg.bodySize);
        doc.setFont('helvetica', 'bold');
        doc.text(sanitizePdfText(primary), cfg.margin, y);
        if (rightText) {
          doc.setFont('helvetica', 'normal');
          doc.text(sanitizePdfText(rightText), pageWidth - cfg.margin, y, { align: 'right' });
        }
        y += cfg.bodySize * cfg.lineHeight;
        if (secondary) {
          doc.setFontSize(cfg.smallSize);
          doc.setFont('helvetica', 'italic');
          const result = addWrappedText(doc, secondary, cfg.margin, y, contentWidth, cfg.smallSize, 'italic', cfg.lineHeight);
          y = result.nextY;
        }
      }
      ...
      doc.text(sanitizePdfText(data.name || 'Generated Resume'), pageWidth / 2, y, { align: 'center' });
      y += cfg.nameSize * 0.9;

      const titleLine = sanitizePdfText(data.position || data.subtitle);
      if (titleLine) {
        const result = addWrappedText(doc, titleLine, pageWidth / 2, y, contentWidth, cfg.bodySize, 'normal', cfg.lineHeight, { align: 'center' });
        y = result.nextY;
      }

      const contactLine = formatContactLine(data);
      if (contactLine) {
        const result = addWrappedText(doc, contactLine, pageWidth / 2, y, contentWidth, cfg.smallSize, 'normal', cfg.lineHeight, { align: 'center' });
        y = result.nextY;
      }
      y += cfg.headerGap;
      ...
      if (data.summary) {
        addSection('Summary');
        const result = addWrappedText(doc, data.summary, cfg.margin, y, contentWidth, cfg.bodySize, 'normal', cfg.lineHeight);
        y = result.nextY + 2;
      }
      ...
      if (skillGroups.some((group) => group.items.length)) {
        ...
        skillGroups.forEach((group) => addLabeledLine(group.label, group.items));
        y += 2;
      }
      ...
      if (data.experience.length) {
        ...
        data.experience.forEach((exp) => {
          const secondary = [exp.company, exp.location].filter(Boolean).join(' | ');
          addHeaderRow(exp.title || exp.company, exp.period, secondary);
      ...
      if (data.projects.length) {
        ...
        data.projects.forEach((project) => {
          const secondary = [project.platform, project.venue].filter(Boolean).join(' | ');
          addHeaderRow(project.title || project.platform, project.period, secondary);
          if (project.points.length) {
            project.points.forEach((point) => addBullet(point));
          } else if (project.description) {
            addBullet(project.description);
          }
          y += cfg.jobGap;
        });
      }
      ...
      if (data.publicationsSummary || data.publications.length) {
        ...
        if (data.publicationsSummary) {
          const result = addWrappedText(doc, data.publicationsSummary, cfg.margin, y, contentWidth, cfg.bodySize, 'normal', cfg.lineHeight);
          y = result.nextY + 2;
        } else {
          data.publications.slice(0, 4).forEach((publication) => {
            const summary = [publication.title, publication.venue, publication.year].filter(Boolean).join(' — ');
            addBullet(summary);
          });
        }
      }
      ...
      if (data.honors.length) {
        ...
        data.honors.slice(0, 6).forEach((honor) => {
          addBullet([honor.year, honor.title, honor.issuer].filter(Boolean).join(' — '));
        });
      }
      ...
      if (data.education.length) {
        ...
        data.education.forEach((education) => {
          const secondary = [education.school, education.location].filter(Boolean).join(' | ');
          addHeaderRow(education.degree || education.school, education.year, secondary);
          education.details.slice(0, 2).forEach((detail) => addBullet(detail));
          y += cfg.jobGap - 1;
        });
      }
    ...
    configs.forEach((cfg) => {
      ...
      render(doc, cfg);
      fallbackDoc = doc;
      if (!outputDoc && doc.internal.getNumberOfPages() === 1) {
        outputDoc = doc;
      }
    });
